    ...
    if(myHud!=None && myHUD.ScoreBoard.IsA('UTComp_ScoreBoard') && gamereplicationinfo!=None)
    {
        if(class'UTComp_Settings'.default.bUseDefaultScoreboard)
        {
            if(gamereplicationinfo.bTeamGame)
                NewScoreboardClass=class'UTCompv18b_k1.UTComp_ScoreBoardTDM';
            else
                NewScoreboardClass=class'UTCompv18b_k1.UTComp_ScoreBoardDM';
            clientChangedScoreboard=True;
        }
    }
    else if(ClientChangedScoreBoard && !class'UTComp_Settings'.default.bUseDefaultScoreboard)
        NewScoreboardClass=class'UTCompv18b_k1.UTComp_ScoreBoard';
...
// This is mostly the original implementation, which the exception of the
// GetFreeMove() call. It has been replaced with GetFreeMoveEx() to fix
// hitching while playing on high fps, and especially with high ping.
...
)
{
	...
	NewMove = GetFreeMoveEx(MAX_SAVED_MOVES);
	if ( NewMove == None )
		return;
...
// Get the next free move, maintaining a maximum of MaxSavedMoves. This is
// created because the original max of 100 in PlayerController.uc causes
// hitches on high fps. By increasing this limit, we reduce the hitching which
// is caused by a loss of position history.
//
// Note: The original GetFreeMove() function cannot be overwritten, hence the
// creation of a new function.
function SavedMove GetFreeMoveEx(int MaxSavedMoves)
{
    ...
    if ( FreeMoves == None )
    {
        // don't allow more than MaxFreeMoves saved moves
...
defaultproperties
{
     ...
     UTCompMenuClass="UTCompv18b_k1.UTComp_Menu_OpenedMenu"
     UTCompVotingMenuClass="UTCompv18b_k1.UTComp_Menu_VoteInProgress"
